  ...
  void receivedImage(const sensor_msgs::Image::ConstPtr& img)
{
    ...
    Mat cpy_img(image.rows, img.cols, image.type());
    ...
    bool flag = findChessboardCorners(image, board_size, corners);
    ...
    {            
      ...
      bitwise_and(cpy_image, image, cpy_image);           // Create a "hole" in the Image to create a "clipping" mask - [ITEM2]           
